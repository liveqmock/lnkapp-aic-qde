...
public class T1070processor extends Stdp10Processor {
    ...
    public void service(Stdp10ProcessorRequest request, Stdp10ProcessorResponse response) throws ProcessorException, IOException {
        ...
        try {
            tia = getTia1070(request.getRequestBody());
        } catch (Exception e) {
            logger.error("特色业务平台请求报文解析错误.", e);
            response.setHeader("rtnCode", TxnRtnCode.MSG_PARSE_FAILED.getCode());
            return;
        }
        ...
        try {
            aictoa1070 = sendAndRecvForAic(getSendMsgForAic(aictia1070));
    ...
    //处理Starring请求报文
    private TIA1070 getTia1070(byte[] body) throws Exception {
